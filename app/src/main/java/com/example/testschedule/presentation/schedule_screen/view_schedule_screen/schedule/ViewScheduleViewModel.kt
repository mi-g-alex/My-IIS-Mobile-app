        ...
        getScheduleUseCase(id = id).onEach { result ->
            when (result) {
                ...
                is Resource.Loading -> {
                    ...
                    a?.let { title.value = a.title }
                }

                is Resource.Error -> {
                    val a = db.getSchedule(id)
                    if (a?.id == lastSelectedSchedule) {
                        _state.value = ViewScheduleState(error = result.message, schedule = a)
                        result.data?.let { title.value = it.title }
                    } else {
                        _state.value = ViewScheduleState(
                            error = result.message,
                            schedule = _state.value.schedule
                        )
                        _state.value.schedule?.let { title.value = it.title }
                    }
                }
            }
