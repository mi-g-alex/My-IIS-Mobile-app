package com.example.testschedule.domain.use_case.account.profile
...
class GetAccountProfileUseCase @Inject constructor(
    ...
    operator fun invoke()  : Flow<Resource<AccountProfileModel>> = flow {
        try {
            emit(Resource.Loading<AccountProfileModel>())
            val cookie = db.getCookie()
            val data = api.getAccountProfile(cookie)
            emit(Resource.Success<AccountProfileModel>(data))
            ...
            try {
                ...
                if (e.toString() == "java.io.EOFException: End of input at line 1 column 1 path \$") {
                    emit(Resource.Error<AccountProfileModel>("WrongPassword"))
                } else if (e.toString().contains("Unable to resolve host")) {
                    emit(Resource.Error<AccountProfileModel>("ConnectionFailed"))
                } else {
                    emit(Resource.Error<AccountProfileModel>("OtherError"))
                }
