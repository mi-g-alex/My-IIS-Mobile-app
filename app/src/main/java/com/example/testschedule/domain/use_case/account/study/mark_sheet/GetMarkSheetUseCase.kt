...
class GetMarkSheetUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Resource<List<MarkSheetModel>>> = flow {
        try {
            ...
            emit(Resource.Success<List<MarkSheetModel>>(data))
            ...
            try {
                ...
                if (e.toString() == "java.io.EOFException: End of input at line 1 column 1 path \$") {
                    emit(Resource.Error<List<MarkSheetModel>>("WrongPassword"))
                } else if (e.toString().contains("Unable to resolve host")) {
                    emit(Resource.Error<List<MarkSheetModel>>("ConnectionFailed"))
                } else {
                    emit(Resource.Error<List<MarkSheetModel>>("OtherError"))
                }
